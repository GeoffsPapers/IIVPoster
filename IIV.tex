...
% \begin{abstract}
% \begin{quote}
% This poster describes the Interactive Interpretation Viewer - IIV, for interpretations of
% typed first order formulae, written in the (new) TPTP format for interpretations.
% \end{quote}
% \end{abstract}
%--------------------------------------------------------------------------------------------------
...
This visualization is available in IIV using the URL provided for the interpretation
...
IDV renders differently, e.g., problem symbols are extracted into annotated formulae with
...
% An ANTLR parser is used to read the extracted annotated formulae, and JavaScript is used to convert
...
This parser is then used in the browser, along with custom JavaScript, to create a graph in the GraphViz {\em DOT} language from an input TPTP file (in this case representing an interpretation).
The resulting DOT text is then rendered using the ``d3-graphviz'' \footnote{https://github.com/magjac/d3-graphviz} library which uses WebAssembly for speed.
